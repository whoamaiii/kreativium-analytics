...
declare global {
  var __WORKER_CAPTURED_MESSAGES__:
    | Array<{ cacheKey?: string; payload?: InsightsWorkerTask['payload'] }>
    | undefined;
}
...
vi.mock('@/workers/analytics.worker?worker', () => {
  class TestWorker {
    ...
    postMessage = vi.fn((msg: Partial<InsightsWorkerTask>) => {
      ...
      globalThis.__WORKER_CAPTURED_MESSAGES__!.push({
...
function Harness({ studentId }: { studentId?: string }) {
  ...
  useEffect(() => {
    ...
    const options = studentId
      ? { student: createStudent(studentId), useAI: false }
      : { student: createStudent('stu-1'), useAI: false };
...
describe('Integration: worker goals positive propagation', () => {
  ...
  it('includes goals in worker payload when student provided', async () => {
    ...
    const workerHasGoals = workerMsgs.some(
      (m) =>
        Array.isArray(m?.payload?.inputs?.goals) &&
        m.payload.inputs.goals.some((g: any) => g.id === 'g1'),
    );
    const fbCalls = (analyticsWorkerFallback.processAnalytics as any).mock?.calls || [];
    const fbHasGoals = fbCalls.some(
      (args: any[]) =>
        Array.isArray(args?.[0]?.goals) && args[0].goals.some((g: any) => g.id === 'g1'),
    );
  ...
  it('fetches goals via inferred student when not provided explicitly', async () => {
    ...
    const workerHasGoals = workerMsgs.some(
      (m) =>
        Array.isArray(m?.payload?.inputs?.goals) &&
        m.payload.inputs.goals.some((g: any) => g.id === 'g2'),
    );
    const fbCalls = (analyticsWorkerFallback.processAnalytics as any).mock?.calls || [];
    const fbHasGoals = fbCalls.some(
      (args: any[]) =>
        Array.isArray(args?.[0]?.goals) && args[0].goals.some((g: any) => g.id === 'g2'),
    );
    expect(workerHasGoals || fbHasGoals).toBe(true);
  });
  ...
  it('fallback path receives goals and forwards them to enhanced analysis', async () => {
    processAnalyticsSpy.mockImplementationOnce(async (data: AnalyticsData) => {
      ...
      await enhancedPatternAnalysis.generatePredictiveInsights(
        [],
        [],
        [createTrackingEntry('t', 'stu-fb')],
        data.goals ?? [],
      );
      return {
        patterns: [],
    ...
    function FBFHarness() {
      const { runAnalysis } = useAnalyticsWorker({ precomputeOnIdle: false });
      useEffect(() => {
        runAnalysis(createAnalyticsData([createTrackingEntry('t', 'stu-fb')]), {
          student: createStudent('stu-fb'),
          useAI: false,
        });
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, []);
  ...
  it('multiple goals are included in worker payload and affect cacheKey', async () => {
    ...
    if (goalsArr && Array.isArray(goalsArr)) {
      ...
      const fbCalls = (analyticsWorkerFallback.processAnalytics as any).mock?.calls || [];
      const fbWithTwo = fbCalls.find(
        (args: any[]) => Array.isArray(args?.[0]?.goals) && args[0].goals.length === 2,
      );

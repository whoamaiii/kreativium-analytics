      ...
      proxy: {
        '/ai': {
          ...
          headers: {
            ...
            Authorization: env.VITE_OPENROUTER_API_KEY
              ? `Bearer ${env.VITE_OPENROUTER_API_KEY}`
              : '',
      ...
      alias: {
        ...
        ...(mode === 'poc'
          ? {
              ...
              '@/workers/analytics.worker': path.resolve(
                __dirname,
                './src/workers/analytics.worker.stub.ts',
              ),
              // Redirect 3D component to a no-op stub in POC mode
              '@/components/Visualization3D': path.resolve(
                __dirname,

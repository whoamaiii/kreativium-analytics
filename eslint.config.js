  ...
  },
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        ...
        {
          selector:
            "CallExpression[callee.property.name='addEventListener'][arguments.length > 1]:not(:has(Identifier[name='useEffect'], Identifier[name='useLayoutEffect']))",
          message:
            'addEventListener should be used inside useEffect with proper cleanup to prevent memory leaks',
        },
        {
          selector:
            "CallExpression[callee.name=/^(setInterval|setTimeout)$/][arguments.length > 0]:not(:has(Identifier[name='useEffect'], Identifier[name='useLayoutEffect']))",
          message:
            'setInterval/setTimeout should be used inside useEffect with proper cleanup (clearInterval/clearTimeout) to prevent memory leaks',
        },
        {
          selector:
            "NewExpression[callee.name='Worker']:not(:has(Identifier[name='useEffect'], Identifier[name='useLayoutEffect']))",
          message:
            'Worker instantiation should be managed through custom hooks with proper cleanup (worker.terminate()) to prevent memory leaks',
        },
        {
          selector:
            "JSXAttribute[name.name='style'][value.type='JSXExpressionContainer'][value.expression.type='ObjectExpression']",
          message:
            'Avoid inline styles (style={{}}). Use Tailwind CSS classes or CSS custom properties for styling. See rule 3wIMH0UDSwsNj5RYGo17Vg. Exception: Radix primitives with // RADIX_INLINE_STYLE_ALLOWED comment.',
        },
      ],

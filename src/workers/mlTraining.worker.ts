...
ctx.onmessage = async (e: MessageEvent<TrainMessage>) => {
  ...
  if (msg.type === 'train-emotion') {
    ctx.postMessage({ type: 'complete', modelType: 'emotion-prediction', metadata: { validationResults: { averageMetrics: {} } } });
    return;
  }
  if (msg.type === 'train-sensory') {
    ctx.postMessage({ type: 'complete', modelType: 'sensory-response', metadata: { validationResults: { averageMetrics: {} } } });
    return;
  }
};

...
export const dataTransformations = {
  ...
  aggregate(data: SessionData[], granularity: 'daily' | 'weekly' | 'monthly' = 'daily'): SessionData[] {
    if (!Array.isArray(data) || data.length === 0) return [];
    ...
    for (const [key, items] of groups.entries()) {
      const avgProgress = items.reduce((s, it) => s + Number(it.progress ?? 0), 0) / Math.max(items.length, 1);
      results.push({ id: `agg-${key}`, studentId: 'aggregate', date: `${key}${granularity === 'daily' ? '' : ''}`, progress: avgProgress });
    }
    return results;
  },
  ...
  filter(
    ...
    return data.filter(d => {
      ...
      const inRange = !dateRange || (t >= dateRange.start.getTime() && t <= dateRange.end.getTime());

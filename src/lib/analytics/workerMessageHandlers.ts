...
const handleAlertsMessage = (
  ...
  if (!payload.prewarm) {
    try {
      log.debug('[ALERTS_PIPELINE] inbound', {
  ...
  const deduped: (AlertEvent | AlertWithGovernance)[] = dedupedWithGovernance.map(
    ({ governance, ...alert }) => ({
      ...alert,
  ...
  newAlerts.forEach((alert) => {
    try {
      ...
      const thresholdAdjustments = safeGetObjectMetadata<Record<string, ThresholdAdjustmentTrace>>(
        metadata,
        'thresholdTrace',
      );
  ...
  if (!payload.prewarm && typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent(ALERTS_UPDATED_EVENT, { detail: { studentId: targetStudentId } }),
    );
  }
...
const handleCompleteMessage = (data: WorkerResponseMessage, ctx: WorkerMessageHandlerContext) => {
  ...
  const shouldUpdateUi =
    !!cacheKeyFromMsg && cacheKeyFromMsg === ctx.activeCacheKeyRef.current && !prewarmFlag;

  if (cacheKeyFromMsg && data.payload) {
    try {
      const storedTags = ctx.cacheTagsRef.current.get(cacheKeyFromMsg);
      const derivedTags =
        storedTags && storedTags.length
          ? storedTags
...
const handleErrorMessage = (data: WorkerResponseMessage, ctx: WorkerMessageHandlerContext) => {
...
export const createWorkerMessageHandlers = (
  ...
  const onMessage = (event: MessageEvent<AnalyticsWorkerMessage>) => {
    ...
    if (!data) {
      log.warn('[workerMessageHandlers] Received invalid worker response message', {
        data: event.data,
      });
      return;
    }
    ...
    try {
      ...
      if (isAlertsResponse(data)) {
        ...
        log.debug('[workerMessageHandlers] Received partial update', {

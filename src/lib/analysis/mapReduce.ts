...
function sampleSnapshot(entries: TrackingEntry[], maxEntries = 8): string {
  ...
  for (const e of pick) {
    ...
    const sn = (e.sensoryInputs || [])
      ...
      .join(', ');
    lines.push(
      `- ${fmtDate(e.timestamp)} | emotions: [${em}] | sensory: [${sn}]${e.notes ? ` | notes: ${e.notes}` : ''}`,
    );
  }
  return lines.join('\n');
}
...
export function chunkByDays<T extends { timestamp: Date }>(
  items: T[],
  start: Date,
  end: Date,
  spanDays: number,
): Array<{ range: TimeRange; items: T[] }> {
  const chunks: Array<{ range: TimeRange; items: T[] }> = [];
  let cursor = new Date(start);
  while (cursor.getTime() < end.getTime()) {
    const chunkStart = new Date(cursor);
    const chunkEnd = new Date(
      Math.min(end.getTime(), chunkStart.getTime() + spanDays * 24 * 60 * 60 * 1000 - 1),
    );
    const inChunk = items.filter(
      (i) =>
        i.timestamp.getTime() >= chunkStart.getTime() &&
        i.timestamp.getTime() <= chunkEnd.getTime(),
    );
...
export async function summarizeChunk(
  ...
  const header =
    `Oppsummer denne perioden kort i norsk JSON. Ingen kodeblokker.\n` +
...
export async function reduceSummariesToFinalReport(
  ...
  try {
    const sys =
      'Svar kun på norsk. Returner KUN gyldig JSON uten kodeblokker. Ikke vis tankestrøm.';
    const contextLines: string[] = [];
    if (opts?.aiProfile) contextLines.push(`AI-profil: ${opts.aiProfile}`);
    ...
    logger.error('[mapReduce] reduceSummariesToFinalReport failed', {
...
export async function analyzeLargePeriod(
  ...
  try {
    const start =
      overallRange.start instanceof Date ? overallRange.start : new Date(overallRange.start);
    ...
    for (const c of entryChunks) {
      const eIn = c.items;
      const emoIn = emotions.filter(
        (e) =>
          e.timestamp.getTime() >= (c.range.start as Date).getTime() &&
          e.timestamp.getTime() <= (c.range.end as Date).getTime(),
      );
      const senIn = sensoryInputs.filter(
        (s) =>
          s.timestamp.getTime() >= (c.range.start as Date).getTime() &&
          s.timestamp.getTime() <= (c.range.end as Date).getTime(),
      );
    ...
    logger.error('[mapReduce] analyzeLargePeriod error', {

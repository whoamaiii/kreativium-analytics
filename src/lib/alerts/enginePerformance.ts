...
import { generateBurstEvents, generateStableBaseline, toTrendSeries } from '@/lib/alerts/detectors/__tests__/syntheticData';

function toDate(ms: number): Date { return new Date(ms); }
...
function buildTracking(n: number, startMs: number): TrackingEntry[] {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    out[i] = {
      ...
      studentId: 'p',
      timestamp: toDate(startMs + i * 60_000),
      emotions: [{ id: `te${i}`, emotion: 'frustration', intensity: i < n / 2 ? 2 : 5, timestamp: toDate(startMs + i * 60_000) } as EmotionEntry],
      sensoryInputs: [],
...
export function benchmarkEngine(datasetSizes: { emotions: number; sensory: number; tracking: number }, iterations = 1): { msAvg: number; alertsPerIter: number } {
...
export function scaleTest(maxStudents = 50, baseSize = { emotions: 600, sensory: 600, tracking: 200 }): Array<{ students: number; msAvg: number }> {
...
export function synthesizeStudentData(points: number): {
  ...
  const emotions: EmotionEntry[] = series.map((p, i) => ({
    id: `e${i}`,
    emotion: 'frustration',
  ...
  const sensoryEvents = generateBurstEvents({ n: Math.max(5, Math.floor(points / 10)), cluster: true, startMs: start });
  const sensory: SensoryEntry[] = sensoryEvents.map((ev, i) => ({
    id: `s${i}`,
    response: 'seeking',
  ...
  const tracking: TrackingEntry[] = series.map((p, i) => ({
    id: `t${i}`,
    studentId: 'synth',
    timestamp: new Date(p.timestamp),
    emotions: [emotions[i]!],
...
export function benchmarkEngineDetailed(studentId: string, points: number): EnginePerfResult {
  ...
  const total = measure(() => engine.runDetection({ studentId, ...data, baseline: baselineService.getBaseline(studentId) })).ms;

  const memMB = typeof (globalThis as any).performance?.memory?.usedJSHeapSize === 'number'
    ? Math.round(((globalThis as any).performance.memory.usedJSHeapSize / (1024 * 1024)) * 100) / 100
    : undefined;

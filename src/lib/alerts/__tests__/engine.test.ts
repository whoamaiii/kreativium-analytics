...
describe('AlertDetectionEngine candidate builders', () => {
  it('builds emotion behavior spike candidates', () => {
    const engine = new AlertDetectionEngine();
    const engineAny = engine as unknown as Record<string, unknown>;
    const emotions = buildEmotions(48, 30);
    const emotionSeries = (engineAny.buildEmotionSeries as (emotions: EmotionEntry[]) => Map<string, { timestamp: number; value: number }[]>)(emotions);
    const overridesRaw = (engineAny.learner as { getThresholdOverrides: () => Record<string, { baselineThreshold?: number; adjustmentValue?: number }> }).getThresholdOverrides();
    const overrides = { ...overridesRaw, beta: { baselineThreshold: 0.05, adjustmentValue: 0 } };
    const candidates = (engineAny.buildEmotionCandidates as (args: Record<string, unknown>) => Array<Record<string, any>>)({
      emotionSeries,
      baseline: null,
      studentId: 's-emotion',
    ...
    expect(candidates[0]?.metadata?.emotionKey).toBe('frustration');
  });

  it('builds sensory behavior spike candidates', () => {
    const engine = new AlertDetectionEngine();
    const engineAny = engine as unknown as Record<string, unknown>;
    const start = Date.UTC(2025, 0, 6, 9, 0, 0);
    const sensory = Array.from({ length: 80 }, (_, idx) => ({
      id: `sens-${idx}`,
      studentId: 's-sensory',
      response: 'touch',
      intensity: idx < 40 ? 1 : 5,
      timestamp: new Date(start + idx * 60_000),
    } as SensoryEntry));
    const aggregates = (engineAny.buildSensoryAggregates as (sensory: SensoryEntry[]) => Map<string, { successes: number; trials: number; delta: number; series: Array<{ timestamp: number; value: number }> }>)(sensory);
    const overridesRaw = (engineAny.learner as { getThresholdOverrides: () => Record<string, { baselineThreshold?: number; adjustmentValue?: number }> }).getThresholdOverrides();
    const overrides = { ...overridesRaw, beta: { baselineThreshold: 0.05, adjustmentValue: 0 } };
    const originalSafeDetect = engineAny.safeDetect as (label: string, fn: () => unknown) => unknown;
    engineAny.safeDetect = (_label: string, fn: () => unknown) => ({
      score: 0.9,
      confidence: 0.85,
      sources: [],
      thresholdApplied: 0.2,
      analysis: fn() ?? {},
    });
    const candidates = (engineAny.buildSensoryCandidates as (args: Record<string, unknown>) => Array<Record<string, any>>)({
      sensoryAggregates: aggregates,
      baseline: null,
      studentId: 's-sensory',
      thresholdOverrides: overrides,
      nowTs: Date.now(),
    });
    engineAny.safeDetect = originalSafeDetect;
    expect(candidates.length).toBeGreaterThan(0);
    expect(candidates[0]?.metadata?.sensoryKey).toBe('touch');
  });

  it('builds association candidates when tracking data correlates', () => {
    const engine = new AlertDetectionEngine();
    const engineAny = engine as unknown as Record<string, unknown>;
    const tracking = buildTracking(90);
    const dataset = (engineAny.buildAssociationDataset as (tracking: TrackingEntry[]) => unknown)(tracking);
    const overrides = (engineAny.learner as { getThresholdOverrides: () => Record<string, unknown> }).getThresholdOverrides();
    const candidates = (engineAny.buildAssociationCandidates as (args: Record<string, unknown>) => Array<Record<string, any>>)({
      dataset,
      studentId: 's-association',
      thresholdOverrides: overrides,
      nowTs: Date.now(),
    });
    expect(candidates.length).toBeGreaterThan(0);
    expect(candidates[0]?.metadata?.contingency).toBeDefined();
  });

  it('builds burst candidates from clustered events', () => {
    const engine = new AlertDetectionEngine();
    const engineAny = engine as unknown as Record<string, unknown>;
    const emotions = buildEmotions(72, 50);
    const sensory = buildSensory(72, 2);
    const events = (engineAny.buildBurstEvents as (emotions: EmotionEntry[], sensory: SensoryEntry[]) => Array<{ timestamp: number; value: number; pairedValue: number }>)(emotions, sensory);
    const overrides = (engineAny.learner as { getThresholdOverrides: () => Record<string, unknown> }).getThresholdOverrides();
    const candidates = (engineAny.buildBurstCandidates as (args: Record<string, unknown>) => Array<Record<string, any>>)({
      burstEvents: events,
      studentId: 's-burst',
      thresholdOverrides: overrides,
      nowTs: Date.now(),
    ...
    expect(candidates.length).toBeGreaterThan(0);
    const eventCount = (candidates[0]?.metadata as { eventCount?: number } | undefined)?.eventCount ?? events.length;

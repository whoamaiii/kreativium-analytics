...
export function computeEwmaControlMultiplier(targetFalseAlertsPerN: number = 336): number {
  const N = Number.isFinite(targetFalseAlertsPerN) && targetFalseAlertsPerN > 1 ? targetFalseAlertsPerN : 336;
  const tail = 1 / (2 * N);
 ...
 */
export function adjustMultiplierByBaselineQuality(multiplier: number, qualityScore?: number): number {

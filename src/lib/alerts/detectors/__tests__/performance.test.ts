...
describe('Detectors performance and Monte Carlo validation', () => {
  it('runs EWMA and CUSUM within reasonable time for 5k points', () => {
    ...
    const tEwma = timeIt('ewma', () =>
      detectEWMATrend(series, { lambda: 0.2, minPoints: 20, targetFalseAlertsPerN: 336 }),
    );
    const tCusum = timeIt('cusum', () =>
      detectCUSUMShift(series, { kFactor: 0.5, minPoints: 20, targetFalseAlertsPerN: 336 }),
    );
  ...
  it('runs BetaRate, Association, and Burst detectors efficiently on large inputs', () => {
    ...
    const betaSample = generateBetaRateData({
      trials: 5000,
      baselineRate: 0.25,
      shift: 0.12,
      priorWeight: 200,
    ...
    const tAssoc = timeIt('association', () =>
      detectAssociation({ label: 'perf', contingency: table, minSupport: 5 }),
    );

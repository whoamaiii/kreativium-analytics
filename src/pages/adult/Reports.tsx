...
export default function AdultReports() {
  ...
  function exportCsv() {
    ...
    const lines = [header.join(',')].concat(rows.map(r => [r.date, r.neutralHolds, r.correctChoices, r.nameItCorrect, r.streak].join(',')));
    ...
    <PinGate>
      <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto space-y-6">
          <h1 className="text-2xl font-semibold">{String(tCommon('adult.reports', { defaultValue: 'Voksensone – rapporter' }))}</h1>
          <button className="px-3 py-2 rounded-lg border" onClick={exportCsv}>{String(tCommon('exportAsCsv', { defaultValue: 'Eksporter som CSV' }))}</button>
        </div>
      </div>
    </PinGate>
  );

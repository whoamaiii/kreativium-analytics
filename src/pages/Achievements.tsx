...
export default function Achievements() {
  ...
  const catalog: Record<StickerId, { label: string; icon: string; goal: number; current: number }> = useMemo(() => ({
    'hold-master': {
  ...
  return (
    <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <h1 className="text-2xl font-semibold">{String(tCommon('achievements.title', { defaultValue: 'Klistremerker' }))}</h1>
            ...
            return (
              <Card key={id} className="p-4 text-center">
                <div className="grid place-items-center gap-2">
                  ...
                  <div className="w-full h-2 rounded bg-muted overflow-hidden" aria-label={`progress ${meta.label}`} role="progressbar" aria-valuemin={0} aria-valuemax={meta.goal} aria-valuenow={meta.current}>
                    <div className="h-full bg-primary" style={{ width: `${Math.round(pct * 100)}%` }} />
                  </div>
                  <div className="text-xs text-muted-foreground">{owned ? String(tCommon('achievements.unlocked', { defaultValue: 'Låst opp!' })) : `${meta.current}/${meta.goal}`}</div>
                </div>
              </Card>
            );
          })}

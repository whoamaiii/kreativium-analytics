...
const Dashboard = () => {
  ...
  const createScopedDashboardTranslator = useCallback(
    (prefix: string) => (key: string, options?: Record<string, unknown>) =>
      tDashboard(`${prefix}.${key}`, options),
    [tDashboard],
  );

  const overviewCopy = useMemo(
    () => createScopedDashboardTranslator('overview'),
    [createScopedDashboardTranslator],
  );
  const studentsCopy = useMemo(
    () => createScopedDashboardTranslator('students'),
    [createScopedDashboardTranslator],
  );
  const actionsCopy = useMemo(
    () => createScopedDashboardTranslator('actions'),
    [createScopedDashboardTranslator],
  );
              ...
              <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      ...
                      <div className="relative bg-gradient-to-br from-primary/20 to-purple-500/20 p-4 rounded-2xl border border-primary/30 group">
                        <div className="text-3xl font-bold text-white motion-safe:group-hover:animate-bounce-slow">
                          {String(tDashboard('brand.initial'))}
                        </div>
                      </div>
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div
                    ...
                    role="toolbar"
                    aria-label={tCommon('accessibility.headerControls')}
                  >
                    <GlobalMenu />
                    <HelpAndSupport />
                    <LanguageSettings />

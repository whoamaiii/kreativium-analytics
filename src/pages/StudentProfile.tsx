...
const headerCls =
  'border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-30';
...
const StudentProfile = () => {
  ...
  const handleSectionChange = useCallback(
    (section: string) => {
      try {
        logger.debug('[UI] Active section change', { from: activeSection, to: section });
      } catch (_err) {
        void 0;
      }
  ...
  useEffect(() => {
    ...
    try {
      if (student) {
        ...
        Promise.resolve(analyticsManager.triggerAnalyticsForStudent(student))
          ...
          .catch((err) => {
            logger.error('[SAFE] analyticsManager.triggerAnalyticsForStudent failed', {
              error: err,
              studentId: student.id,
            });
          });
        ...
        if (import.meta.env.DEV) {
          logger.debug('[DIAGNOSTIC] analyticsManager.initializeStudentAnalytics start', {
            studentId,
          });
        }
        try {
          ...
          logger.error('[SAFE] analyticsManager.initializeStudentAnalytics failed', {
            error: err,
            studentId,
          });
        }
    ...
    return () => {
      ...
      if (import.meta.env.DEV) {
        logger.debug(
          '[DIAGNOSTIC] StudentProfile cleanup: aborting controller and clearing timers if any',
        );
      }
      controller.abort();
    };
        ...
        switch (format) {
          case 'pdf': {
            ...
            const allDates: Date[] = [
              ...
              ...filteredData.sensoryInputs.map(
                (s) => (s as unknown as { timestamp: Date }).timestamp,
              ),
            ];
          ...
          <header className={headerCls}>
            <div className={headerInnerCls}>
              <div className={flexGap3Cls}>
                <SidebarTrigger />
                <Button
                  variant={BTN_VARIANT_GHOST}
                  size={BTN_SIZE_SM}
                  onClick={() => navigate('/')}
                  aria-label={tCommon('aria.go_back_to_dashboard')}
                >
              ...
              {activeSection === 'goals' && (
                <div className={spaceY6Cls}>
                  <div>
                    ...
                    <p className={textMutedCls}>{t('goals_description', { name: student.name })}</p>
                  </div>

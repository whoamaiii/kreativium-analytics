...
function blendshapeToEmotion(blendshapes: Array<{ categoryName: string; score: number }>): {
...
export class MediaPipeDetector {
  ...
  async tick(): Promise<void> {
    ...
    if (result && result.facialBlendshapes && result.facialBlendshapes.length > 0) {
      const blend = result.facialBlendshapes[0].categories.map((c: any) => ({

...
function ComparisonSummaryComponent({
  ...
  const [lastExport, setLastExport] = useState<{ type: 'csv' | 'json' | null; at: number | null }>({
    type: null,
    at: null,
  });
  ...
  const content = useMemo(() => {
    ...
    const interventions = diffInterventions(
      (current as any).suggestedInterventions || [],
      (baseline as any).suggestedInterventions || [],
    );
    const keyFindings = diffKeyFindings(
      (current as any).keyFindings || [],
      (baseline as any).keyFindings || [],
    );
  ...
  useEffect(() => {
    ...
    const buildPatternsCSV = () => {
      ...
      content.patterns.changed.forEach((c) =>
        rows.push(
          `changed,"${c.current?.name || c.baseline?.name}",${(c.deltaStrength ?? '').toString()},${c.impactChange?.from ?? ''},${c.impactChange?.to ?? ''}`,
        ),
      );
      return rows.join('\n');
    };
    const buildCorrelationsCSV = () => {
      const rows: string[] = [
        'type,variables,delta_coefficient,direction_changed,significance_changed',
      ];
      content.correlations.added.forEach((c) => rows.push(`added,"${c.variables.join(' × ')}",,,`));
      content.correlations.removed.forEach((c) =>
        rows.push(`removed,"${c.variables.join(' × ')}",,,`),
      );
      content.correlations.changed.forEach((c) =>
        rows.push(
          `changed,"${(c.current?.variables || c.baseline?.variables)?.join(' × ')}",${(c.deltaCoefficient ?? '').toString()},${c.directionChanged ? '1' : '0'},${c.significanceCrossed ? '1' : '0'}`,
        ),
      );
      return rows.join('\n');
    };
    ...
    const onCsv = () => {
      ...
      downloadText(
        `${prefix}_correlations_delta.csv`,
        'text/csv;charset=utf-8',
        buildCorrelationsCSV(),
      );
      downloadText(
        `${prefix}_interventions_delta.csv`,
        'text/csv;charset=utf-8',
      ...
      <CardContent className={cn('space-y-4', hasSmallBaseline && 'opacity-75')}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-3 text-sm">
          <div className="rounded-md border p-3">
            ...
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.counts.patterns.deltaPct}
              />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.correlations')}
            </div>
            <div className="flex items-center justify-between">
              <div className="font-medium">{current.correlations?.length ?? 0}</div>
              <Trend
                value={content.empty ? 0 : (content as any).summary.counts.correlations.delta}
              />
            </div>
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.counts.correlations.deltaPct}
              />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.insights')}
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.counts.insights.deltaPct}
              />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.anomalies')}
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.counts.anomalies.deltaPct}
              />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.predictive')}
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.counts.predictive.deltaPct}
              />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.balance')}
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="mt-0.5">
              <TrendPct value={content.empty ? 0 : (content as any).summary.balance?.deltaPct} />
            </div>
          </div>
          <div className="rounded-md border p-3">
            <div className="text-[11px] text-muted-foreground">
              {tAnalytics('interface.avgIntensityShort')}
            </div>
            <div className="flex items-center justify-between">
              <div className="font-medium">
                {currentAvgIntensity ? currentAvgIntensity.toFixed(1) : '—'}
              </div>
              <Trend value={content.empty ? 0 : (content as any).summary.avgIntensity?.delta} />
            </div>
            <div className="mt-0.5">
              <TrendPct
                value={content.empty ? 0 : (content as any).summary.avgIntensity?.deltaPct}
              />
            </div>
          </div>
        </div>
        ...
        {!content.empty && (
          <div className="space-y-2">
            ...
            {topChangedPatterns.length === 0 &&
            content.patterns.added.length === 0 &&
            content.patterns.removed.length === 0 ? (
              <div className="text-sm text-muted-foreground">
                {tAnalytics('interface.identicalResults')}
              </div>
            ) : (
                  ...
                  <div key={c.key} className="rounded-md border p-3">
                    ...
                    {c.impactChange && (
                      <div className="mt-1 text-[11px] text-muted-foreground inline-flex items-center gap-1">
                        ...
                        {tAnalytics('interface.impactChange')} {String(c.impactChange.from || '—')}{' '}
                        → {String(c.impactChange.to || '—')}
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="flex items-center justify-between gap-2">
                <TabsList>
                  ...
                  <TabsTrigger value="interventions">
                    {tAnalytics('interface.newInterventions')}
                  </TabsTrigger>
                </TabsList>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          aria-label={String(tAnalytics('interface.exportCsvAria'))}
                        >
                          {String(tAnalytics('interface.exportCsv'))}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        {String(tAnalytics('interface.exportCsvTooltip'))}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          aria-label={String(tAnalytics('interface.copyJsonAria'))}
                        >
                          {String(tAnalytics('interface.copyJson'))}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        {String(tAnalytics('interface.copyJsonTooltip'))}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                  {lastExport.at && (
                    <span className="text-[11px] text-muted-foreground ml-1">
                      {String(tAnalytics('interface.lastExportShort'))}:{' '}
                      {lastExport.type?.toUpperCase()}{' '}
                      {new Date(lastExport.at).toLocaleTimeString()}
                    </span>
                  )}
                  <Button
                    variant="ghost"
                    ...
                    onClick={() => {
                      ...
                      setCorrLimit(6);
                      setIntAddedLimit(8);
                      setIntRemovedLimit(8);
                      setLastExport({ type: null, at: null });
                      try {
                        const s = (studentName || 'elev').toLowerCase();
                        const c = currentLabel.toLowerCase();
                        const b = baselineLabel.toLowerCase();
                  ...
                  return (
                    <div className="space-y-3">
                      <div className="text-sm font-medium">
                        {tAnalytics('interface.keyChanges')}
                      </div>
                      ...
                      {changed.length > patLimit && (
                        <div className="mt-1">
                          <Button
                            size="sm"
                            variant="outline"
                            aria-label="Vis flere endrede mønstre"
                            onClick={() => setPatLimit((p) => p + 8)}
                          >
                            Vis flere
                          </Button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <div className="text-sm font-medium mb-1">
                              {tAnalytics('interface.newPattern')}
                            </div>
                            <ul className="text-sm space-y-1">
                              {added.length === 0 ? (
                                <li className="text-muted-foreground">
                                  {tAnalytics('interface.none')}
                                </li>
                              ) : (
                                added.map((a) => <li key={a.name}>+ {a.name}</li>)
                              )}
                            </ul>
                          </div>
                          <div>
                            <div className="text-sm font-medium mb-1">
                              {tAnalytics('interface.removedPattern')}
                            </div>
                            <ul className="text-sm space-y-1">
                              {removed.length === 0 ? (
                                <li className="text-muted-foreground">
                                ...
                                removed.map((r) => <li key={r.name}>– {r.name}</li>)
                              )}
                            </ul>
                          </div>
                        </div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {changed.slice(0, corrLimit).map((c) => (
                          <div key={c.key} className="rounded-md border p-3 text-sm">
                            <div className="font-medium">
                              {c.baseline?.variables.join(' × ') ||
                                c.current?.variables.join(' × ')}
                            </div>
                            <div className="mt-1 flex items-center justify-between text-[13px]">
                              <div>
                                {tAnalytics('interface.deltaRPrefix')}{' '}
                                {(c.deltaCoefficient ?? 0).toFixed(2)}
                              </div>
                              {c.significanceCrossed && (
                                <Badge variant="outline">
                                  {tAnalytics('interface.significanceChanged')}
                                </Badge>
                              )}
                            </div>
                          </div>
                      ...
                      {changed.length > corrLimit && (
                        <div className="mt-1">
                          <Button
                            size="sm"
                            variant="outline"
                            aria-label="Vis flere korrelasjonsendringer"
                            onClick={() => setCorrLimit((p) => p + 8)}
                          >
                            Vis flere
                          </Button>
                        </div>
                          ...
                          <div>
                            ...
                            <ul className="text-sm space-y-1">
                              {added.length === 0 ? (
                                ...
                                added.map((a) => <li key={a.key}>+ {a.variables.join(' × ')}</li>)
                              )}
                            </ul>
                          </div>
                          <div>
                            ...
                            <ul className="text-sm space-y-1">
                              {removed.length === 0 ? (
                                ...
                                removed.map((r) => <li key={r.key}>– {r.variables.join(' × ')}</li>)
                              )}
                            </ul>
                          </div>
                        </div>
                  ...
                  return (
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              ...
                              {added.slice(0, intAddedLimit).map((i) => (
                                <li key={i.title} className="flex items-center justify-between">
                                  ...
                                  {typeof i.confidenceOverall === 'number' && (
                                    <Badge variant="outline">
                                      {Math.round(i.confidenceOverall * 100)}%
                                    </Badge>
                                  )}
                                </li>
                              ))}
                          ...
                          {added.length > intAddedLimit && (
                            <div className="mt-1">
                              <Button
                                size="sm"
                                variant="outline"
                                aria-label="Vis flere nye tiltak"
                                onClick={() => setIntAddedLimit((p) => p + 8)}
                              >
                                Vis flere
                              </Button>
                            </div>
                        ...
                        <div className="rounded-md border p-3">
                          <div className="text-sm font-medium mb-2">
                            {tAnalytics('interface.removedInterventions')}
                          </div>
                          {removed.length === 0 ? (
                            <div className="text-sm text-muted-foreground">
                              {tAnalytics('interface.none')}
                          ...
                          {removed.length > intRemovedLimit && (
                            <div className="mt-1">
                              <Button
                                size="sm"
                                variant="outline"
                                aria-label="Vis flere fjernede tiltak"
                                onClick={() => setIntRemovedLimit((p) => p + 8)}
                              >
                                Vis flere
                              </Button>
                            </div>
                ...
                <div>
                  ...
                  <ul className="list-disc pl-5">
                    {(content.keyFindings.added || []).map((s) => (
                      <li key={`added-${s}`} className="text-emerald-700">
                        + {s}
                      </li>
                    ))}
                    {(content.keyFindings.removed || []).map((s) => (
                      <li key={`removed-${s}`} className="text-rose-700">
                        – {s}
                      </li>
                    ))}
                  </ul>
                </div>
                {(content as any).summary.recencyMs &&
                  (content as any).summary.recencyMs.delta !== null && (
                    <div>
                      <div className="font-medium mb-1">
                        {tAnalytics('interface.analysisTimestamp')}
                      </div>
                      <div className="text-sm text-muted-foreground">
                        {tAnalytics('interface.timestampDifference')}:{' '}
                        <Trend value={(content as any).summary.recencyMs.delta} /> ms
                      </div>
                    </div>
                  )}
              </div>

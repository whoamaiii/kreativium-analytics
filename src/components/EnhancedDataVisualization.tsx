...
const processChartData = (
  entries: (EmotionEntry | SensoryEntry)[],
  type: 'emotions' | 'sensory',
  days: number = 30,
): DataPoint[] => {
...
export const EnhancedDataVisualization = ({
  ...
  const uniqueKeys =
    activeData.length > 0 ? Object.keys(activeData[0]).filter((k) => k !== 'date') : [];
      ...
      <Card className="p-6 font-sans shadow-lg bg-card">
        <div className="flex justify-center items-center h-full">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm">
              There is no {dataType} data available for {studentName}.
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-card-foreground">
            Enhanced Data Insights for {studentName}
          </h2>
          <p className="text-muted-foreground">Displaying {dataType}</p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            onClick={() => setDataType('emotions')}
            variant={dataType === 'emotions' ? 'default' : 'secondary'}
          >
            Emotions
          </Button>
          <Button
            onClick={() => setDataType('sensory')}
            variant={dataType === 'sensory' ? 'default' : 'secondary'}
          >
            Sensory
          </Button>
        </div>

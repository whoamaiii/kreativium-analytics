...
const TrendsChartLazy = lazy(() =>
  import('./charts/TrendsChart').then((m) => ({ default: m.TrendsChart })),
);
const CorrelationHeatmapLazy = lazy(() =>
  import('./analysis/CorrelationHeatmap').then((m) => ({ default: m.CorrelationHeatmap })),
);
import { LazyPatternsPanel } from '@/components/lazy/LazyPatternsPanel';
const EChartContainerLazy = lazy(() =>
...
export const InteractiveDataVisualization = memo<InteractiveDataVisualizationProps>(
  ({
    ...
    const safeInitialEmotions = useMemo(
      () => (Array.isArray(initialEmotions) ? initialEmotions : []),
      [initialEmotions],
    );
    const safeInitialSensoryInputs = useMemo(
      () => (Array.isArray(initialSensoryInputs) ? initialSensoryInputs : []),
      [initialSensoryInputs],
    );
    const safeInitialTracking = useMemo(
      () => (Array.isArray(initialTrackingEntries) ? initialTrackingEntries : []),
      [initialTrackingEntries],
    );

    const availableEmotions = useMemo(
      () => Array.from(new Set(safeInitialEmotions.map((e) => e.emotion))),
      [safeInitialEmotions],
    );
    ...
    const realtimeData = useRealtimeData(
      ...
      },
      {
        ...
        windowSize:
          visualizationState.selectedTimeRange === '7d'
            ? 7 * 24 * 60
            : visualizationState.selectedTimeRange === '30d'
              ? 30 * 24 * 60
              : visualizationState.selectedTimeRange === '90d'
                ? 90 * 24 * 60
                : 0,
        updateInterval: 1000,
    ...
    const sensoryInputs = filterCriteria.realtime
      ? realtimeData.sensoryInputs
      : safeInitialSensoryInputs;
    const trackingEntries = filterCriteria.realtime
      ? realtimeData.trackingEntries
      : safeInitialTracking;
    ...
    const currentStudent = useMemo(
      () => ({ id: currentStudentId, name: studentName }) as Student,
    ...
    useEffect(() => {
      ...
      const next = {
        e: filteredData.emotions.length,
        s: filteredData.sensoryInputs.length,
        t: filteredData.trackingEntries.length,
      };
      ...
      filteredData.trackingEntries.length,
    ]);
    ...
    const chartData = useMemo(() => {
      ...
      filteredData.emotions.forEach((emotion) => {
        ...
        data.emotionCount++;
        data.avgEmotionIntensity =
          (data.avgEmotionIntensity * (data.emotionCount - 1) + emotion.intensity) /
          data.emotionCount;
    ...
    const togglePictureInPicture = useCallback(() => {
      ...
      toast({
        title: visualizationState.isPictureInPicture
      ...
      try {
        ...
        const studentData: Student = {
          ...
          baselineData: {
            ...
            environmentalFactors: { optimalConditions: {}, challengingConditions: [] },
        ...
        const exportData: AnalyticsExportData = {
          ...
          analytics: {
            ...
            correlations:
              analysisData.correlationMatrix?.significantPairs.map((p) => ({
                ...p,
                id: crypto.randomUUID(),
                description: '',
                recommendations: [],
              })) || [],
            insights: analysisData.patterns.map((p) => p.description),
      ...
      switch (type) {
          ...
          if (visualizationState.projectionMode === '2d') {
            ...
            const mapPoint = (p: (typeof chartData)[number]) => {
              ...
              return {
                name: p.date,
                value: [p.timestamp, y],
                avgEmotionIntensity: p.avgEmotionIntensity,
                totalSensoryInputs: p.totalSensoryInputs,
              };
            };
        ...
        case 'correlations':
          return (
            ...
            >
              <CorrelationHeatmapLazy
                correlationMatrix={analysisData.correlationMatrix}
                onRetry={() => {}}
                onShowAllTime={() => visualizationState.setSelectedTimeRange('all')}
              />
            </Suspense>
          );
        case 'patterns':
        ...
        case 'timeline':
          return (
            <TimelineVisualization
              ...
              sensoryInputs={filteredData.sensoryInputs}
              trackingEntries={filteredData.trackingEntries}
              anomalies={analysisData.anomalies.map((a) => ({
                timestamp: a.timestamp,
                type: a.type,
                severity: a.severity,
              }))}
              onTimeRangeChange={(start, end) =>
                visualizationState.setFilterCriteria((prev) => ({
                  ...prev,
                  dateRange: { start, end },
                }))
              }
        ...
        <div ref={containerRef} className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              aria-label={String(
                tAnalytics('actions.refreshInsights', { defaultValue: 'Refresh insights' }),
              )}
              title={String(
                tAnalytics('actions.refreshInsights', { defaultValue: 'Refresh insights' }),
              )}
            ...
            <Button
              ...
              aria-label={String(
                tAnalytics('actions.autoRefresh', { defaultValue: 'Auto refresh' }),
              )}
            >
          ...
          {error && !isAnalyzing && (
            <div
              className="p-4 border border-destructive rounded-md"
              role="alert"
              aria-live="assertive"
            >
          ...
          {layoutMode === 'dashboard' && (
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-4 items-start">
              ...
              <TeacherInsightsPanel
                ...
                onCreateGoal={() => {
                  /* wire later: route to goals with draft */
                }}
                onAddIntervention={() => {
                  /* wire later */
                }}
          ...
          {layoutMode === 'grid' && (
            <GridLayout
              ...
              selectedVisualizations={visualizationState.selectedVisualizations}
            />
          )}
          {layoutMode === 'focus' && (
            <FocusLayout
              renderVisualization={renderVisualization}
              focusedVisualization={visualizationState.focusedVisualization}
            />
          )}
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      (prevProps.emotions.length === 0 ||
        nextProps.emotions.length === 0 ||
        prevProps.emotions[0]?.timestamp?.getTime() ===
          nextProps.emotions[0]?.timestamp?.getTime()) &&
      (prevProps.sensoryInputs.length === 0 ||
        nextProps.sensoryInputs.length === 0 ||
        prevProps.sensoryInputs[0]?.timestamp?.getTime() ===
          nextProps.sensoryInputs[0]?.timestamp?.getTime()) &&
      (prevProps.trackingEntries.length === 0 ||
        nextProps.trackingEntries.length === 0 ||
        prevProps.trackingEntries[0]?.timestamp?.getTime() ===
          nextProps.trackingEntries[0]?.timestamp?.getTime())
    );
  },
);

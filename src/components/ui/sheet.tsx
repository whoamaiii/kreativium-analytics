...
const hasSheetTitle = (children: React.ReactNode): boolean => {
  ...
  const walk = (node: React.ReactNode) => {
    ...
    React.Children.forEach(node as React.ReactNode, (child) => {
      ...
      if ((child.type as unknown) === SheetPrimitive.Title) {
        found = true;
        return;
      }
...
const hasSheetDescription = (children: React.ReactNode): boolean => {
  ...
  const walk = (node: React.ReactNode) => {
    ...
    React.Children.forEach(node as React.ReactNode, (child) => {
      if (found) return;
      if (!React.isValidElement(child)) return;
      if ((child.type as unknown) === SheetPrimitive.Description) {
        found = true;
        return;
      }
      const props: any = (child as any)?.props;
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        {...props}
      >
        {ensureTitle && <SheetPrimitive.Title className="sr-only">Menu</SheetPrimitive.Title>}
        {ensureDescription && (

...
function determineOptimalChartType(filteredData: OverviewPanelProps['filteredData']): ChartType {
  ...
  const emotionTypes = new Set<string>(
    filteredData.emotions.map((e) => (e?.emotion || '').toLowerCase()).filter(Boolean),
  );
  const sensoryTypes = new Set<string>(
    filteredData.sensoryInputs
      .map((s) => (s?.sensoryType || s?.type || '').toLowerCase())
      .filter(Boolean),
  );
...
export const OverviewPanel = memo(
        ...
        <section aria-labelledby="overview-title" className="space-y-6">
          <header className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground" aria-live="polite">
              {optimalChartType === 'line' &&
                String(
                  tAnalytics('charts.timeSeriesDetected', { defaultValue: 'Time series view' }),
                )}
              {optimalChartType === 'area' &&
                String(
                  tAnalytics('charts.categoricalDetected', { defaultValue: 'Distribution view' }),
                )}
              {optimalChartType === 'composed' &&
                String(tAnalytics('charts.mixedDataDetected', { defaultValue: 'Combined view' }))}
            </p>
          </header>

          <div
            aria-label={String(
              tAnalytics('aria.overview.heroChart', {
                defaultValue: 'Overview chart showing key metrics',
              }),
            )}
            aria-live="polite"
          >
            <Suspense
              fallback={
                <div
                  ...
                  aria-label={String(
                    tAnalytics('overview.loadingChart', {
                      defaultValue: 'Loading overview chart...',
                    }),
                  )}
                />
              }
            >
              <LazyChartsPanel
                ...
                preferredChartType={optimalChartType}
              />
            </Suspense>
          </div>
          ...
          <section aria-labelledby="overview-insights-title" className="space-y-3">
            ...
            {topInsights.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {String(
                  tAnalytics('overview.noInsights', {
                    defaultValue: 'No insights available yet. More data will improve analysis.',
                  }),
                )}
              </p>
            ) : (
              <ul
                ...
                aria-label={String(
                  tAnalytics('aria.overview.insights', {
                    defaultValue: 'Key insights from data analysis',
                  }),
                )}
              >
                {topInsights.map((insight, idx) => (
                  <li key={idx} className="text-sm">
                    ...
                    <button
                      ...
                      aria-label={String(
                        tAnalytics('aria.overview.insightLink', {
                          defaultValue: 'View detailed analysis in explore tab',
                        }),
                      )}
                    >
                      {String(
                        tAnalytics('overview.viewInExplore', { defaultValue: 'View in Explore' }),
                      )}
                    </button>
                  </li>
                ))}
          ...
          <section aria-labelledby="overview-patterns-title" className="space-y-3">
            <h3 id="overview-patterns-title" className="text-base font-medium">
              {String(
                tAnalytics('overview.detectedPatterns', { defaultValue: 'Detected Patterns' }),
              )}
            </h3>
            {topPatterns.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {String(
                  tAnalytics('overview.noPatternsYet', {
                    defaultValue: 'No patterns detected yet.',
                  }),
                )}
              </p>
            ) : (
                ...
                {topPatterns.map((p) => {
                  ...
                  return (
                    <Card
                      ...
                      aria-label={String(
                        tAnalytics('aria.overview.openPattern', {
                          defaultValue: 'Open pattern explanation',
                        }),
                      )}
                    >
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-1">
                            ...
                            <div className="text-xs text-muted-foreground">
                              {String(tAnalytics('overview.patternType', { defaultValue: 'Type' }))}
                              : {String(p.type)}
                            </div>
                            {p.description && (
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-[11px] text-muted-foreground">
                              {String(
                                tAnalytics('insights.confidence', { defaultValue: 'Confidence' }),
                              )}
                            </div>
                          </div>
                        </div>
  ...
  (prev, next) => {
    ...
    const prevFirstTs =
      prev.filteredData.entries[0]?.timestamp?.getTime() ||
      prev.filteredData.emotions[0]?.timestamp?.getTime() ||
      prev.filteredData.sensoryInputs[0]?.timestamp?.getTime() ||
      0;
    const nextFirstTs =
      next.filteredData.entries[0]?.timestamp?.getTime() ||
      next.filteredData.emotions[next.filteredData.emotions.length - 1]?.timestamp?.getTime() ||
      next.filteredData.sensoryInputs[
        next.filteredData.sensoryInputs.length - 1
      ]?.timestamp?.getTime() ||
      0;
    const prevLastTs =
      prev.filteredData.entries[prev.filteredData.entries.length - 1]?.timestamp?.getTime() ||
      prev.filteredData.emotions[prev.filteredData.emotions.length - 1]?.timestamp?.getTime() ||
      prev.filteredData.sensoryInputs[
        prev.filteredData.sensoryInputs.length - 1
      ]?.timestamp?.getTime() ||
      0;
    const nextLastTs =
      next.filteredData.entries[next.filteredData.entries.length - 1]?.timestamp?.getTime() ||
      next.filteredData.emotions[next.filteredData.emotions.length - 1]?.timestamp?.getTime() ||
      next.filteredData.sensoryInputs[
        next.filteredData.sensoryInputs.length - 1
      ]?.timestamp?.getTime() ||

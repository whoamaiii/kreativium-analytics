...
export const OverviewPanel = memo(function OverviewPanel({ studentName, filteredData, insights }: OverviewPanelProps): React.ReactElement {
  ...
  const nextFirstTs = next.filteredData.entries[0]?.timestamp?.getTime() || next.filteredData.emotions[next.filteredData.emotions.length - 1]?.timestamp?.getTime() || next.filteredData.sensoryInputs[next.filteredData.sensoryInputs.length - 1]?.timestamp?.getTime() || 0;

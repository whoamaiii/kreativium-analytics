...
export function ExplanationDock({
  ...
  const readiness = React.useMemo(() => {
    try {
      ...
      return evaluateSocialTriggerReadiness(
        dataset.entries as any,
        dataset.emotions as any,
        dataset.sensoryInputs as any,
      );
    } catch {
      return null;
    }
      ...
      <CardContent className="h-[calc(100vh-8rem)] 2xl:h-[calc(100vh-6rem)] overflow-hidden">
        {readiness && (
          <details
            ...
            aria-label="Data readiness for sosiale triggere"
          >
            <summary className="cursor-pointer select-none">
              ...
              >
                Data readiness for sosiale triggere: {Math.round(readiness.score * 100)}% (
                {readiness.label})
              </span>
            </summary>
                ...
                {isDev && dataset?.entries?.[0]?.studentId && (
                  <div className="mt-2">
                    <Button
                      ...
                      size="sm"
                      onClick={async () => {
                        await seedSocialDemoData(dataset.entries[0].studentId);
                      }}
                    >

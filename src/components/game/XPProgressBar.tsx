...
export function XPProgressBar({ progress, xp, xpToNext, level, streak, pendingDelta = 0, leveledUp = false, disableParticles = false, onLevelUp, className }: XPProgressBarProps) {
  ...
  const glow = Math.min(0.9, 0.15 + Math.max(0, streak) * 0.05 + Math.min(0.5, pendingDelta * 0.01));
  ...
  const orbsCount = useMemo(() => Math.max(0, Math.min(10, 2 + Math.floor((pendingDelta || 0) / 5))), [pendingDelta]);
  ...
  return (
    <div className={cn('w-full', className)}>
      <div className="flex items-baseline justify-between mb-1">
        ...
        <div className="text-xs text-foreground/70">{xp} / {xpToNext} XP</div>
      </div>
      <div
        ...
        style={{ background: trackColor, boxShadow: `0 0 0 1px rgba(255,255,255,0.05) inset, 0 0 16px rgba(16,185,129,${glow * 0.25})` }}
      >
        <motion.div
          className="absolute left-0 top-0 bottom-0 rounded-full"
          style={{ background: `linear-gradient(90deg, ${barColor}, ${barColor})`, boxShadow: `0 0 14px ${barColor}` }}
          initial={{ width: 0 }}
          animate={{ width: widthTarget }}
          transition={reduced ? { duration: 0.1 } : { type: 'spring', stiffness: 220, damping: 26, mass: 0.6 }}
        />

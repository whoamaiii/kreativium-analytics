    ...
    <AnimatePresence>
      {visible && (
        <motion.div className="fixed inset-0 z-40" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            <h2 className="text-2xl font-bold mb-3">{String(tCommon('game.calibration.title', { defaultValue: 'Calibration' }))}</h2>
            <p className="text-sm text-muted-foreground">{String(tCommon('game.calibration.description', { defaultValue: 'Stay neutral for about 10–15 seconds while we calibrate your camera and lighting.' }))}</p>
            <div className="mt-4 h-2 w-full rounded bg-white/10 overflow-hidden" aria-label="Calibration progress" role="progressbar" aria-valuenow={Math.round(progress * 100)} aria-valuemin={0} aria-valuemax={100}>
              <div className="h-full bg-emerald-400" style={{ width: `${Math.round(progress * 100)}%` }} />
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button className="rounded-md bg-white/10 px-4 py-2 hover:bg-white/15 transition" onClick={finish}>{String(tCommon('game.calibration.finish', { defaultValue: 'Finish' }))}</button>
              <button className="rounded-md bg-white/10 px-4 py-2 hover:bg-white/15 transition" onClick={onClose}>{String(tCommon('game.calibration.skip', { defaultValue: 'Skip' }))}</button>
            </div>
          </motion.div>
        </motion.div>
      )}

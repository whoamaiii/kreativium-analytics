...
export function MatchMeter({ value }: { value: number }) {
  ...
  return (
    <div className="w-full max-w-xs" aria-label="Match meter">
      <div className="h-2 w-full rounded bg-white/15 overflow-hidden" role="progressbar" aria-valuenow={Math.round(v * 100)} aria-valuemin={0} aria-valuemax={100}>
        <div className={`h-full transition-all duration-200 ${color}`} style={{ width: `${Math.round(v * 100)}%` }} />
      </div>

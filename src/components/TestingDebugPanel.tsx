...
export const TestingDebugPanel = ({ className = '' }: TestingDebugPanelProps) => {
  ...
  const runSystemTests = async () => {
    ...
    try {
      ...
      const testStatus = analyticsManager
        .getAnalyticsStatus()
        .find((s) => s.studentId === testStudentId);

      results[2] = {
        name: 'New Student Flow',
        status: testStatus?.isInitialized && !testStatus?.hasMinimumData ? 'pass' : 'fail',
        message:
          testStatus?.isInitialized && !testStatus?.hasMinimumData
      ...
      results[5] = {
        ...
        message: cacheStats
          ? `Hit rate: ${cacheStats.hitRate.toFixed(1)}%, Size: ${cacheStats.size} items`
  ...
  const getStatusColor = useCallback(
    (status: TestStatus): 'default' | 'destructive' | 'secondary' | 'outline' => {
      switch (status) {
        case 'pass':
          return 'default';
        case 'fail':
  ...
  const handleInvalidateStudent = useCallback(
    (studentId: string) => {
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">
                      {cacheStats.size} / {analyticsWorker.cacheSize}
                    </span>
                  </div>
                  <div className="flex flex-col">

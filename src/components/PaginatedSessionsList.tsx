...
export const PaginatedSessionsList = memo(
  ({ sessions, title = 'Sessions in Selected Period' }: PaginatedSessionsListProps) => {
    const {
      paginatedData,
      currentPage,
      pageSize,
    ...
    return (
      <Card className="bg-gradient-card border-0 shadow-soft">
        ...
        <CardContent className="space-y-4">
          {isLoading ? (
            <div className="text-center p-8 text-muted-foreground">Loading sessions...</div>
          ) : (
            <div>
              {/* Sessions List */}
              <div className="space-y-4">
                {paginatedData.map((entry) => (
                  <div
                    key={entry.id}
                    className="border-l-4 border-primary pl-4 py-2 hover:bg-muted/50 transition-colors rounded-r"
                  >

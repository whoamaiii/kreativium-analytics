...
export const DataQualityFeedback = memo(
  ({ emotions, sensoryInputs, entries, className }: DataQualityFeedbackProps) => {
    ...
    const qualityMetrics = useMemo(() => {
      ...
      const daysWithData = allDays.filter((day) => {
        return allTimestamps.some(
          (timestamp) => format(timestamp, 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd'),
        );
      });
      ...
      const completeSessions = entries.filter(
        (entry) =>
      ...
      const completenessScore =
        totalEntries > 0 ? (completeSessions.length / totalEntries) * 100 : 0;
      ...
      return (
        <Card className={className}>
          ...
          <CardContent className="text-center py-8">
            ...
            <p className="text-muted-foreground">Ingen data tilgjengelig for kvalitetsvurdering</p>
          </CardContent>
        </Card>
      );
    }
        ...
        <CardHeader>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={
                  overallScore >= 80 ? 'default' : overallScore >= 60 ? 'secondary' : 'outline'
                }
              >
                {overallScore}/100
              </Badge>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-950/20 dark:to-blue-950/20 rounded-lg">
            ...
            <p className="text-sm text-muted-foreground mb-3">
              {overallScore >= 90 &&
                'Utmerket datakvalitet! Dine data gir høy sikkerhet for analyser.'}
              {overallScore >= 70 &&
                overallScore < 90 &&
                'God datakvalitet. Små forbedringer kan øke sikkerhetsnivået.'}
              {overallScore >= 50 &&
                overallScore < 70 &&
                'Moderat datakvalitet. Fokuser på de områdene som trenger forbedring.'}
              {overallScore < 50 &&
                'Datakvaliteten kan forbedres betydelig. Følg anbefalingene for bedre analyser.'}
            </p>

            {/* Top Priority Recommendations */}
            {qualityMetrics.filter((m) => m.status === 'poor' || m.status === 'fair').length >
              0 && (

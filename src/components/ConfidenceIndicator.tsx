...
const ConfidenceIndicatorComponent = memo<ConfidenceIndicatorProps>(
  ({ confidence, dataPoints = 0, timeSpanDays = 0, rSquared = 0, className = '' }) => {
    ...
    const generateExplanations = (): string[] => {
      ...
      if (dataPoints < 10) {
        explanations.push(
          String(
            tAnalytics('confidence.explanations.insufficientData', {
              count: dataPoints,
              minimum: 10,
            }),
          ),
      ...
      if (timeSpanDays < 14) {
        explanations.push(
          String(
            tAnalytics('confidence.explanations.shortTimespan', {
              days: timeSpanDays,
              recommended: 21,
            }),
          ),
      ...
      if (rSquared < 0.3) {
        explanations.push(
          String(
            tAnalytics('confidence.explanations.weakPattern', {
        ...
        explanations.push(
          String(
            tAnalytics('confidence.explanations.strongPattern', {
              rsquared: rSquared.toFixed(3),
            }),
          ),
        );
        ...
        <Tooltip>
          ...
          <TooltipContent side="bottom" className="max-w-xs">
            <div className="space-y-2">
              <div className="font-medium">{String(tAnalytics(`confidence.${level}`))}</div>

    ...
    formatter: function (params: unknown) {
      ...
      const getEmotionIcon = (name: string | undefined) => {
        ...
        return typeof name === 'string'
          ? icons[name.toLowerCase()] || icons.default
          : icons.default;
      };
      const seriesNameToKey = (name?: string): string | undefined => {
        if (!name) return undefined;
        const n = name.toLowerCase();
        if (n.includes(String(tAnalytics('charts.avgEmotionIntensity')).toLowerCase()))
          return 'avgEmotionIntensity';
        if (n.includes('7d') || n.includes('ma')) return 'avgEmotionIntensityMA';
        if (n.includes(String(tAnalytics('charts.positiveEmotions')).toLowerCase().split(' ')[0]))
          return 'positiveEmotions';
        if (n.includes(String(tAnalytics('charts.negativeEmotions')).toLowerCase().split(' ')[0]))
          return 'negativeEmotions';
        if (n.includes(String(tAnalytics('charts.sensoryInputs')).toLowerCase().split(' ')[0]))
          return 'totalSensoryInputs';
        return undefined;
      };
      ...
      const valueFromParam = (p: TP) => {
        ...
        if (
          (p as TP)?.value &&
          typeof (p as TP).value === 'object' &&
          !Array.isArray((p as TP).value)
        ) {
        ...
        if ((p as TP)?.data && typeof (p as TP).data === 'object') {
          if (typeof yKey === 'string' && yKey in ((p as TP).data as Record<string, unknown>))
            return ((p as TP).data as Record<string, unknown>)[yKey] as number;
        }
...
function EChartContainerBase({
  ...
  const themed = useMemo(() => {
    try {
      ...
      const xAxis = (safeOption as EChartsOption).xAxis as
        | XAXisComponentOption
        | XAXisComponentOption[]
        | undefined;
      const yAxis = (safeOption as EChartsOption).yAxis as
        | YAXisComponentOption
        | YAXisComponentOption[]
        | undefined;
      ...
      const themedOption: EChartsOption = withGlobalSeriesDefaults({
        ...
        tooltip: {
          ...baseTheme.tooltip,
          ...(safeOption as EChartsOption).tooltip,
      ...
      if (import.meta?.env?.DEV) {
        logger.error('[EChartContainer] Theme merge failed', {
          ...
          optionPreview: (() => {
            try {
              return JSON.stringify(safeOption, (_k, v) =>
  ...
  return (
    <div
      ...
      style={{
        // RADIX_INLINE_STYLE_ALLOWED - ECharts container needs explicit overflow control
...
function EChartContainerWithExport(
  props: EChartContainerProps & { exportRegistration?: EChartExportRegistration },
) {
  ...
  React.useEffect(() => {
    ...
    props.exportRegistration?.id,
    props.exportRegistration?.title,
    props.exportRegistration?.studentId,
    props.exportRegistration?.type,
    props.exportRegistration?.filters,
    props.exportRegistration?.dateRange,
    exportMethods,
  ]);

  return <EChartContainerBase {...props} __innerChartRef__={innerRef} />;
}
